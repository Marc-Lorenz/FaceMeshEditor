...
var _bootstrap = require("bootstrap"); // import statically - don't grab it from a cdn
...
var _faceLandmarksFeatures = require("./graph/face_landmarks_features");
...
class App {
    constructor(cacheSize){
        this.fileCache = [];
        this.editor = new (0, _editor2D.Editor2D)();
        this.models = {
            mediapipe: {
                model: new (0, _mediapipe.MediapipeModel)(),
                selected: true
            },
            custom: {
                model: null,
        ...
        this.featureDrag = new (0, _slider.Slider)("feature_drag", ()=>{
            // TODO FIX Not working!
            const element = document.getElementById("num");
            element.value = this.featureDrag.getValue().toString();
            this.editor.dragDepth = this.featureDrag.getValue();
        });
        this.viewTesselation = new (0, _checkbox.CheckBox)("view_tesselation", ()=>this.editor.showTesselation = this.viewTesselation.isChecked());
        ...
        this.editor.setOnBackgroundLoadedCallback((_)=>{
            if (this.getSelectedFileHistory()?.isEmpty()) this.runDetection();
            else this.editor.graph = this.getSelectedFileHistory()?.get();
        });
    }
    openImage() {
        ...
        input.onchange = ()=>{
            if (input.files) {
                ...
                for (const f of files){
                    const history = new (0, _fileAnnotationHistory.FileAnnotationHistory)(f, this.cacheSize);
                    this.fileCache.push(history);
                    const thumbnail = new (0, _thumbnail.Thumbnail)((filename)=>this.selectThumbnail(filename));
            ...
            if (input.files && input.files.length > 0) {
                ...
                reader.onload = (_)=>{
                    ...
                    for (const filename of Object.keys(jsonString)){
                        const graph = (0, _graph.Graph).fromJson(jsonString[filename], (id)=>new (0, _point2D.Point2D)(id, 0, 0, []));
                        const cache = this.fileCache.find((f)=>f.file.name === filename);
                        if (cache) {
                            cache.add(graph);
                            if (this.selectedFile === filename) this.editor.graph = graph;
                        }
                    }
                    this.editor.draw();
                };
    ...
    saveAnnotation() {
        if (this.fileCache.length > 0) {
            ...
            for (const c of this.fileCache){
                const graph = c.get();
                if (graph) result[c.file.name] = graph.toDictArray();
            }
            ...
            case (0, _models.ModelType).custom:
                {
                    ...
                    (0, _webservice.WebServiceModel).verifyUrl(url).then((error)=>{
                        ...
                        if (error === null) {
                            ...
                            // Display error:
                            switch(error){
                                case (0, _webservice.urlError).InvalidUrl:
                                    errorText.removeAttr("hidden");
                                    errorText.text("Please enter a valid URL!");
                                    break;
                                case (0, _webservice.urlError).Unreachable:
                                    errorText.removeAttr("hidden");
    ...
    getModel() {
        for(const modelName in this.models){
            if (this.models[modelName].selected) return this.models[modelName].model;
        }
        return undefined;
    }
    deleteFeature(feature) {
        ...
        switch(feature){
            case "left_eye":
                this.deletePoints((0, _faceLandmarksFeatures.FACE_FEATURE_LEFT_EYE));
                break;
            case "left_eyebrow":
                this.deletePoints((0, _faceLandmarksFeatures.FACE_FEATURE_LEFT_EYEBROW));
                break;
            case "right_eye":
                this.deletePoints((0, _faceLandmarksFeatures.FACE_FEATURE_RIGHT_EYE));
                break;
            case "right_eyebrow":
                this.deletePoints((0, _faceLandmarksFeatures.FACE_FEATURE_RIGHT_EYEBROW));
                break;
            case "nose":
                this.deletePoints((0, _faceLandmarksFeatures.FACE_FEATURE_NOSE));
                break;
            case "mouth":
                this.deletePoints((0, _faceLandmarksFeatures.FACE_FEATURE_LIPS));
                break;
            default:
    ...
    selectThumbnail(filename) {
        ...
        if (cache) this.editor.setBackgroundSource(cache.file);
    }
    resizeWindow() {
        this.editor.draw();
    }
    runDetection() {
        this.getModel()?.detect(this.getSelectedFileHistory().file).then((graph)=>{
    ...
    deletePoints(pointIds) {
        ...
        if (graph) {
            for (const id of pointIds)graph.getById(id).deleted = true;
            this.editor.graph = graph;
        }
    }
}
...
window.onload = (_)=>{
    ...
    document.getElementById("btnModelMediapipe").onclick = ()=>app.setModel((0, _models.ModelType).mediapipe);
    document.getElementById("btnCloseModal").onclick = ()=>app.setModel((0, _models.ModelType).mediapipe);
    document.getElementById("btnCancelModal").onclick = ()=>app.setModel((0, _models.ModelType).mediapipe);
    document.getElementById("btnSaveCustomModel").onclick = ()=>app.setModel((0, _models.ModelType).custom);
    ...
    window.onwheel = (e)=>{
        if (e.shiftKey) app.addFeatureDrag(e.deltaY / 100);
    };
};

},{"bootstrap":"h36JB","./view/slider":"2FkMd","./view/checkbox":"ee2Da","./view/thumbnail":"hYn0c","./cache/fileAnnotationHistory":"hIwLx","./graph/point2d":"gKi6j","./editor2d":"jTDww","./graph/graph":"6zOyc","./graph/face_landmarks_features":"b4de7","./model/mediapipe":"nRDbt","./model/models":"ayqrf","./model/webservice":"8LoVW","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"h36JB":[function(require,module,exports) {
/*!
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"2FkMd":[function(require,module,exports) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"ee2Da":[function(require,module,exports) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hYn0c":[function(require,module,exports) {
/**
...
class Thumbnail {
   ...
   */ setSource(file) {
        ...
        reader.onload = ()=>{
            ...
            if (result) this.image.src = result.toString();
        };
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hIwLx":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 ...
 */ parcelHelpers.export(exports, "FileAnnotationHistory", ()=>FileAnnotationHistory);
class FileAnnotationHistory {
   ...
   */ add(item) {
        if (this.currentHistoryIndex + 1 < this.history.length) // Delete history stack when moved back and changed something
        this.history.length = this.currentHistoryIndex + 1;
        if (this.cacheSize === this.history.length) // Remove the first item as it is too old and cache limit is reached
        this.history.shift();
   ...
   */ setIndex(index) {
        if (index < 0) index = 0;
        else if (index >= this.history.length) index = this.history.length - 1;
   ...
   */ get() {
        if (!this.isEmpty()) return this.history[this.currentHistoryIndex];
        return null;
    }
    /**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gKi6j":[function(require,module,exports) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"jTDww":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "Editor2D", ()=>Editor2D);
var _point2D = require("./graph/point2d");
...
class Editor2D {
    constructor(){
        ...
        this._dragDepth = 0;
        this._graph = new (0, _graph.Graph)([]);
    ...
    draw() {
        ...
        // Draw Mesh
        if (this.showTesselation) this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_TESSELATION, COLOR_EDGES_TESSELATION);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_FACE_OVAL, COLOR_EDGES_FACE_OVAL);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LIPS, COLOR_EDGES_LIPS);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_EYEBROW, COLOR_EDGES_RIGHT_EYE);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_EYE, COLOR_EDGES_RIGHT_EYE);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_IRIS, COLOR_EDGES_RIGHT_IRIS);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_EYEBROW, COLOR_EDGES_LEFT_EYE);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_EYE, COLOR_EDGES_LEFT_EYE);
        this.drawFaceTrait((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_IRIS, COLOR_EDGES_LEFT_IRIS);
        this.drawFaceTrait((0, _faceLandmarksFeatures.FACE_LANDMARKS_NOSE), COLOR_EDGES_NOSE);
    }
    drawPoint(point) {
        if (point && !point.deleted) {
            const projectedPoint = (0, _perspective2D.Perspective2D).project(this.image, point);
            if (point.hovered) {
                this.ctx.beginPath();
        ...
        if (this.graph) {
            ...
            for (const connection of pointPairs){
                ...
                if (startPoint && endPoint && !startPoint.deleted && !endPoint.deleted) {
                    startPoint = (0, _perspective2D.Perspective2D).project(this.image, startPoint);
                    endPoint = (0, _perspective2D.Perspective2D).project(this.image, endPoint);
    ...
    handleMouseDown(event) {
        ...
        else if (event.button === 1) // wheel button
        this.isPanning = true;
    }
    handleMouseMove(event) {
        ...
        if (this.isMoving) {
            ...
            const relativeMouse = (0, _perspective2D.Perspective2D).unproject(this.image, new (0, _point2D.Point2D)(-1, relativeMouseX, relativeMouseY, []));
            ...
            for(let depth = 0; depth <= this.dragDepth; depth++){
                ...
                for (const neigP of neighbourPoints){
                    ...
                    const newPoint = new (0, _point2D.Point2D)(-1, newX, newY, []);
            ...
            // Redraw
            this.draw();
        } else if (this.isPanning) this.pan(this.mouseX - this.prevMouseX, this.mouseY - this.prevMouseY);
        else if (this.image) {
            let pointHover = false;
            const relativeMouse = (0, _perspective2D.Perspective2D).unproject(this.image, new (0, _point2D.Point2D)(-1, relativeMouseX, relativeMouseY, []));
            this._graph.points.forEach((point)=>{
                if (!pointHover && (0, _perspective2D.Perspective2D).intersects(this.image, point, relativeMouse, POINT_EXTENDED_WIDTH / this.zoomScale)) {
                    point.hovered = true;
                    pointHover = true;
                } else {
            ...
            if (pointHover) this.draw();
        }
    }
    handleMouseUp(_) {
        if (this.isMoving && this.onPointsEditedCallback) this.onPointsEditedCallback(this._graph);
...
},{"./graph/point2d":"gKi6j","./graph/perspective2d":"24fdq","./graph/graph":"6zOyc","@mediapipe/tasks-vision":"e5Mjq","./graph/face_landmarks_features":"b4de7","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"24fdq":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 * Represents a utility class for 2D perspective transformations.
 */ parcelHelpers.export(exports, "Perspective2D", ()=>Perspective2D);
class Perspective2D {
    /**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"6zOyc":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"e5Mjq":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"b4de7":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 ...
 */ parcelHelpers.export(exports, "findNeighbourPointIds", ()=>findNeighbourPointIds);
parcelHelpers.export(exports, "FACE_FEATURE_LIPS", ()=>FACE_FEATURE_LIPS);
parcelHelpers.export(exports, "FACE_FEATURE_LEFT_EYE", ()=>FACE_FEATURE_LEFT_EYE);
parcelHelpers.export(exports, "FACE_FEATURE_LEFT_EYEBROW", ()=>FACE_FEATURE_LEFT_EYEBROW);
parcelHelpers.export(exports, "FACE_FEATURE_RIGHT_EYE", ()=>FACE_FEATURE_RIGHT_EYE);
parcelHelpers.export(exports, "FACE_FEATURE_RIGHT_EYEBROW", ()=>FACE_FEATURE_RIGHT_EYEBROW);
parcelHelpers.export(exports, "FACE_LANDMARKS_NOSE", ()=>FACE_LANDMARKS_NOSE);
parcelHelpers.export(exports, "FACE_FEATURE_NOSE", ()=>FACE_FEATURE_NOSE);
var _tasksVision = require("@mediapipe/tasks-vision");
class Connection {
    /**
...
function findNeighbourPointIds(pointId, connections, depth) {
    if (depth === 0) return Array.from(new Set([
        pointId
    ]));
    const neighbours = connections.filter((conn)=>conn.start === pointId || conn.end === pointId).map((conn)=>conn.start === pointId ? conn.end : conn.start);
    const neighbourIds = new Set(neighbours);
    for (const neighbour of neighbours){
        const subNeighbours = findNeighbourPointIds(neighbour, connections, depth - 1);
        for (const subNeighbour of subNeighbours)neighbourIds.add(subNeighbour);
    }
    return Array.from(neighbourIds);
}
const FACE_FEATURE_LIPS = Array.from(new Set((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LIPS.map((con)=>con.start).concat([
    62,
    76,
    184,
    183,
...
const FACE_FEATURE_LEFT_EYE = Array.from(new Set((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_EYE.map((con)=>con.start).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_EYE.map((con)=>con.end)).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_IRIS.map((con)=>con.start).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_IRIS.map((con)=>con.end)))));
const FACE_FEATURE_LEFT_EYEBROW = Array.from(new Set((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_EYEBROW.map((con)=>con.start).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_LEFT_EYEBROW.map((con)=>con.end))));
const FACE_FEATURE_RIGHT_EYE = Array.from(new Set((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_EYE.map((con)=>con.start).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_EYE.map((con)=>con.end)).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_IRIS.map((con)=>con.start).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_IRIS.map((con)=>con.end)))));
const FACE_FEATURE_RIGHT_EYEBROW = Array.from(new Set((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_EYEBROW.map((con)=>con.start).concat((0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_RIGHT_EYEBROW.map((con)=>con.end))));
const FACE_LANDMARKS_NOSE = convertToConnections([
    2,
    97
], [
    97,
...
const FACE_FEATURE_NOSE = Array.from(new Set(FACE_LANDMARKS_NOSE.map((con)=>con.start).concat(FACE_LANDMARKS_NOSE.map((con)=>con.end)).concat([
    102,
    49,
    209,
    217,
...
},{"@mediapipe/tasks-vision":"e5Mjq","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"nRDbt":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 ...
 */ parcelHelpers.export(exports, "MediapipeModel", ()=>MediapipeModel);
var _graph = require("../graph/graph");
var _faceLandmarksFeatures = require("../graph/face_landmarks_features");
var _tasksVision = require("@mediapipe/tasks-vision");
...
class MediapipeModel {
   ...
   */ constructor(){
        (0, _tasksVision.FilesetResolver).forVisionTasks("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm").then((filesetResolver)=>(0, _tasksVision.FaceLandmarker).createFromOptions(filesetResolver, {
                baseOptions: {
    ...
    async detect(imageFile) {
        return new Promise((resolve, reject)=>{
            ...
            image.onload = (_)=>{
                ...
                if (result) {
                    const graphs = result.faceLandmarks.map((landmarks)=>landmarks.map((dict, idx)=>{
                            const ids = Array.from((0, _faceLandmarksFeatures.findNeighbourPointIds)(idx, (0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_TESSELATION, 1));
                            return new (0, _point3D.Point3D)(idx, dict.x, dict.y, dict.z, ids);
                        }).map((point)=>point)).map((landmarks)=>new (0, _graph.Graph)(landmarks));
                    if (graphs) resolve(graphs[0]);
                } else reject("Face(s) could not be detected!");
            };
            const reader = new FileReader();
            reader.onload = (_)=>{
                const result = reader.result;
                if (result) image.src = result.toString();
                else reject("Image could not be read!");
            };
...
},{"../graph/graph":"6zOyc","../graph/face_landmarks_features":"b4de7","@mediapipe/tasks-vision":"e5Mjq","../graph/point3d":"65pXd","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"65pXd":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
class Point3D extends (0, _point2D.Point2D) {
    /**
...
},{"./point2d":"gKi6j","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"ayqrf":[function(require,module,exports) {
/** Specifies the models in the API */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "ModelType", ()=>ModelType);
var ModelType;
(function(ModelType) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"8LoVW":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 * Represents a model using a WebService for face landmark detection.
 * Implements the ModelApi interface for working with Point2D graphs.
 */ parcelHelpers.export(exports, "WebServiceModel", ()=>WebServiceModel);
parcelHelpers.export(exports, "urlError", ()=>urlError);
var _point2D = require("../graph/point2d");
var _graph = require("../graph/graph");
var _faceLandmarksFeatures = require("../graph/face_landmarks_features");
var _tasksVision = require("@mediapipe/tasks-vision");
class WebServiceModel {
   ...
   */ constructor(url){
    ...
    async detect(imageFile) {
        ...
        return fetch(request).then((res)=>res.json()).then((landmarks)=>landmarks.map((dict, idx)=>{
                const ids = Array.from((0, _faceLandmarksFeatures.findNeighbourPointIds)(idx, (0, _tasksVision.FaceLandmarker).FACE_LANDMARKS_TESSELATION, 1));
                return new (0, _point2D.Point2D)(idx, dict.x, dict.y, ids);
            })).then((landmarks)=>new (0, _graph.Graph)(landmarks));
    }
   ...
   */ static async verifyUrl(url) {
        ...
        if (!pattern.test(url)) return "InvalidUrl";
...
},{"../graph/point2d":"gKi6j","../graph/graph":"6zOyc","../graph/face_landmarks_features":"b4de7","@mediapipe/tasks-vision":"e5Mjq","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["1G5n2","kuM8f"], "kuM8f", "parcelRequired82d")
